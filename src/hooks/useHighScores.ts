...
export const useHighScores = (): UseHighScoresReturn => {
    ...
    (scores: HighScore[]) => {
      ...
      if (currentFilter.level !== undefined) {
        filtered = filtered.filter(
          (score) => score.level === currentFilter.level,
        );
      }

      if (currentFilter.difficulty) {
        filtered = filtered.filter(
          (score) => score.difficulty === currentFilter.difficulty,
        );
      }
  ...
  const calculateStats = useCallback((scores: HighScore[]): HighScoreStats => {
    ...
    const levelCounts = scores.reduce(
      (acc, s) => {
  ...
  const loadHighScores = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to load high scores";
  ...
  const saveHighScores = useCallback(
    async (scores: HighScore[], newStats: HighScoreStats) => {
      try {
  ...
  const addHighScore = useCallback(
    async (scoreData: Omit<HighScore, "id" | "timestamp">) => {
  ...
  // Delete high score
  const deleteHighScore = useCallback(
    async (id: string) => {
  ...
  const importScores = useCallback(
    async (data: string) => {
      try {
        ...
        const validScores: HighScore[] = importData.highScores.map(
          (score: Record<string, unknown>) =>
            ({
              ...score,
              timestamp: new Date(score.timestamp as string),
              id: (score.id as string) || crypto.randomUUID(),
            }) as HighScore,
        );
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to import scores";
        setError(errorMessage);
