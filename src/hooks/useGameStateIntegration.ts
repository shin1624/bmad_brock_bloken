...
import { PowerUpType, ActivePowerUp } from '../components/game/HUD/PowerUpStatus';
import { PowerUpAnimationEvent, PowerUpAnimationType } from '../components/game/animations/PowerUpAnimations';
...
interface GameStateIntegration {
  ...
  onPowerUpPickup: (powerUpId: string, position: { x: number; y: number }) => void;
...
export const useGameStateIntegration = (
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (updateCounter.current % 20 === 0) { // Assuming 50ms interval = 20 updates/second
        setDebugInfo(prev => ({
  ...
  const getPowerUpMetadata = (type: PowerUpType) => {
    const metadataMap: Record<PowerUpType, { icon: string; color: string; name: string }> = {
      [PowerUpType.MultiBall]: { icon: '⚡', color: '#ff6b6b', name: 'Multi Ball' },
      [PowerUpType.PaddleSize]: { icon: '🏓', color: '#4ecdc4', name: 'Paddle Size' },
      [PowerUpType.BallSpeed]: { icon: '💨', color: '#45b7d1', name: 'Ball Speed' },
      [PowerUpType.Penetration]: { icon: '🎯', color: '#96ceb4', name: 'Penetration' },
      [PowerUpType.Magnet]: { icon: '🧲', color: '#feca57', name: 'Magnet' }
    };
    return metadataMap[type] || { icon: '❓', color: '#999999', name: 'Unknown' };
  };
  ...
  // Event handlers
  const onPowerUpSpawn = useCallback((powerUp: GamePowerUpState) => {
  ...
  const onPowerUpPickup = useCallback((powerUpId: string, position: { x: number; y: number }) => {
  ...
  const onPowerUpActivate = useCallback((powerUpId: string) => {
    const powerUp = powerUpRegistry.current.get(powerUpId);
    if (!powerUp) return;
    ...
    if (finalConfig.debugMode) {
      console.log(`PowerUp activated: ${powerUp.type} (${powerUpId}) for ${powerUp.timeRemaining}ms`);
    }
  }, [finalConfig.animationEnabled, finalConfig.debugMode]);

  const onPowerUpExpire = useCallback((powerUpId: string) => {
    const powerUp = powerUpRegistry.current.get(powerUpId);
    ...
    if (finalConfig.debugMode) {
      console.log(`PowerUp expired: ${powerUp?.type || 'unknown'} (${powerUpId})`);
    }
  }, [finalConfig.animationEnabled, finalConfig.debugMode]);
