...
describe('Power-Up System Performance', () => {
  ...
  describe('PowerUp Entity Performance', () => {
    ...
    it('should maintain consistent performance over time', () => {
      ...
      expect(maxFrameTime).toBeLessThan(PERFORMANCE_CONFIG.MAX_FRAME_TIME_MS * 2);
      expect(framesExceedingBudget).toBeLessThanOrEqual(PERFORMANCE_CONFIG.ACCEPTABLE_FRAME_DROPS);
    });
  });
  ...
  describe('PowerUpSystem Performance', () => {
    it('should handle maximum active effects efficiently', async () => {
      ...
      const mockPlugins = Object.values(PowerUpType).map(type => ({
        ...
        removeEffect: vi.fn().mockReturnValue({ success: true, modified: true }),
        updateEffect: vi.fn().mockReturnValue({ success: true, modified: false }),
      ...
      for (let i = 0; i < 8; i++) { // Default max active effects
        const type = Object.values(PowerUpType)[i % Object.values(PowerUpType).length];
    ...
    it('should maintain 60 FPS with complex effect combinations', async () => {
      ...
      const mockPlugins = Object.values(PowerUpType).map(type => ({
        ...
        effect: {
          ...
          conflictsWith: Math.random() > 0.7 ? [Object.values(PowerUpType)[0]] : [],
          apply: vi.fn(),
          remove: vi.fn()
        },
        init: vi.fn().mockResolvedValue(undefined),
      ...
      for (let frame = 0; frame < 60; frame++) {
        ...
        if (Math.random() > 0.7) {
          const type = Object.values(PowerUpType)[
            Math.floor(Math.random() * Object.values(PowerUpType).length)
          ];
          await powerUpSystem.applyEffect(type, `powerup-${frame}`, mockGameState);
        }
  ...
  describe('Memory Performance', () => {
    ...
    it('should cleanup expired effects efficiently', async () => {
      ...
      const mockPlugin = {
        ...
        effect: { id: 'test', priority: 5, stackable: true, apply: vi.fn(), remove: vi.fn() },
        init: vi.fn().mockResolvedValue(undefined),
        destroy: vi.fn().mockResolvedValue(undefined),
        applyEffect: vi.fn().mockReturnValue({ success: true, modified: true }),
        removeEffect: vi.fn().mockReturnValue({ success: true, modified: true }),
      ...
      for (let i = 0; i < 50; i++) {
        await testSystem.applyEffect(PowerUpType.MultiBall, `powerup-${i}`, mockGameState);
      }
