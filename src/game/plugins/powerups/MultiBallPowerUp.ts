...
export class MultiBallPowerUp extends PowerUpPlugin {
  ...
  protected onApplyEffect(context: PowerUpPluginContext): EffectResult {
    try {
      ...
      this.log(`MultiBall effect applied: added ${ballsToAdd} balls (total: ${balls.length})`);
  ...
  protected onRemoveEffect(context: PowerUpPluginContext): EffectResult {
    try {
      ...
      for (let i = balls.length - 1; i >= 0; i--) {
        const ball = balls[i];
        if (ball && (ball as any).id && effectData.addedBallIds.includes((ball as any).id)) {
          // Safely remove ball
          ball.active = false;
          balls.splice(i, 1);
          removedCount++;
      ...
      this.log(`MultiBall effect removed: removed ${removedCount} balls (remaining: ${balls.length})`);
  ...
  private createClonedBall(originalBall: Ball, cloneIndex: number): Ball {
    ...
    const config: BallConfiguration = {
      ...
      initialSpeed: originalBall.speed * (1 + (Math.random() - 0.5) * MultiBallPowerUp.SPEED_VARIATION),
    ...
    const originalAngle = Math.atan2(originalBall.velocity.y, originalBall.velocity.x);
    const angleVariation = (cloneIndex * MultiBallPowerUp.SPAWN_ANGLE_SPREAD / 2) - 
  ...
  private rollbackEffect(context: PowerUpPluginContext): void {
    try {
      ...
      this.log(`Failed to rollback MultiBall effect: ${error.message}`, 'error');
    }
  }
