...
const ComboDisplay: React.FC<ComboDisplayProps> = ({
  ...
  useEffect(() => {
    ...
    setIsComboExpiring(
      combo.timeRemaining < maxComboTime * 0.3 && combo.count > 0,
    );
  ...
  const containerStyle: React.CSSProperties = {
    ...
    animation: isComboExpiring
      ? "comboExpiring 0.5s infinite alternate"
      : isHighStreak
        ? "comboGlow 1s ease-in-out infinite alternate"
        : "none",
  ...
  const timeFillStyle: React.CSSProperties = {
    ...
    height: "100%",
    backgroundColor: isComboExpiring
      ? "#ff4757"
      : isHighStreak
        ? "#ffd700"
        : "#00ff88",
  ...
  return (
    <div style={containerStyle}>
      <div style={comboCountStyle}>{combo.count}</div>

      <div style={multiplierStyle}>{formatMultiplier(combo.multiplier)}</div>

      {combo.streak >= 5 && (
        <div style={streakLabelStyle}>{getStreakLevel(combo.streak)}</div>
      )}
