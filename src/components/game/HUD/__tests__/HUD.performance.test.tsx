...
describe("HUD Performance Tests", () => {
  ...
  it("should handle lives updates efficiently", () => {
    const { rerender } = render(
      <LivesDisplay lives={3} maxLives={5} />
    );
  ...
  it("should handle level progression updates efficiently", () => {
    const { rerender } = render(
      <LevelDisplay level={1} progress={0} />
    );
  ...
  it("should handle combo updates efficiently", () => {
    const { rerender } = render(
      <ComboDisplay combo={{ count: 0, multiplier: 1, streak: 0, timeRemaining: 5000 }} />
    );
  ...
  it("should maintain performance with complex HUD state", () => {
    const complexPowerUps: ActivePowerUp[] = Array.from({ length: 5 }, (_, i) => ({
      id: `complex-powerup-${i}`,
      type: ['multiball', 'paddlesize', 'ballspeed', 'penetration', 'magnet'][i],
      duration: 10000 - (i * 1000),
      maxDuration: 10000,
      icon: ['⚡', '🏓', '💨', '🎯', '🧲'][i],
      color: ['#ff6b6b', '#4ecdc4', '#45b7d1', '#96ceb4', '#feca57'][i],
      name: ['Multi Ball', 'Big Paddle', 'Speed Boost', 'Penetration', 'Magnet'][i]
    }));
